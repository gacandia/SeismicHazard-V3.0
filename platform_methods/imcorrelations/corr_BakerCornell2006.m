function [rho] = corr_BakerCornell2006(T1, T2, direction,~)
...
%   direction   = 'default' 0 for correlation in the same axis
%               = 'perpendicular' for correlation on perpendicular axes
%
% OUTPUT
%
...
for i=1:numel(T_min)
    rho(i)=corrModel(T_min(i), T_max(i),direction);
end
rho(T_min<0.05)=nan;
rho(T_max>5)  =nan;

function rho=corrModel(T_min,T_max,direction)

X = 1 - log(T_max/T_min)*(0.32 - 0.133*exp( -(log(T_max)+1.51)^2 -5*(log(T_min)+2.59)^2));
D = 9 - log(T_max/T_min^4);
rho = max([X, X-0.17*D, 0.15]);

if strcmp(direction,'perpendicular')
