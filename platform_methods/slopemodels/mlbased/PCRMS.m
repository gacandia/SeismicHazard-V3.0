function [vout] = PCRMS(ky, T, Sa,PGV,M,MDCOL,varargin)

scaler  = MDCOL{1}; %py.joblib.load("D:\SeismicHazardPlatform - BETA\SeismicHazardPlatform - BETA\platform_methods\slopemodels\slab\pcr_transform_x.joblib");
pca     = MDCOL{2}; %py.joblib.load("D:\SeismicHazardPlatform - BETA\SeismicHazardPlatform - BETA\platform_methods\slopemodels\slab\pca_transform.joblib");
mdl     = MDCOL{3}; %py.joblib.load("D:\SeismicHazardPlatform - BETA\SeismicHazardPlatform - BETA\platform_methods\slopemodels\slab\pcr_model.joblib");
lnky    = log(ky);
lnPGV   = log(PGV);
lnSa    = log(Sa);
