...
isadmisible = imag(To)==0 && or(To>=0 && To<=5,To==-1);
if ~isadmisible
    lny   = [];
    sigma = [];
    tau   = [];
...
if T_lo==T_hi
    [log10y,sigma,tau,phi] = gmpe(index,M,Rrup,Rhyp,Zhyp,direction);
else
    [log10y_lo,sigma_lo,tau_lo] = gmpe(index,  M,Rrup,Rhyp,Zhyp,direction);
    [log10y_hi,sigma_hi,tau_hi] = gmpe(index+1,M,Rrup,Rhyp,Zhyp,direction);
    x          = log([T_lo;T_hi]);
...
log10y       = c1+c2*M+c3*R-c4*log10(R)+c5*Zhyp;
tau          = se*ones(size(M));
phi          = sr*ones(size(M));
