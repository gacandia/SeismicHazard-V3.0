...
isadmisible = imag(To)==0 && or(To>=0 && To<=10,or(To==-1,To==-2));
if ~isadmisible
    lny   = [];
    sigma = [];
    tau   = [];
...
if T_lo==T_hi
    [lny,sigma,tau,phi] = gmpe(index,M, Rrup, Rjb, Ztor, dip, SOF, VS30, Z25, sigmatype);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M, Rrup, Rjb, Ztor, dip, SOF, VS30, Z25, sigmatype);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M, Rrup, Rjb, Ztor, dip, SOF, VS30, Z25, sigmatype);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
switch sigmatype
    case 'arbitrary', sigmatype=1;
    case 'average',   sigmatype=0;
end
