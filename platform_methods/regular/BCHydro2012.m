...
if T_lo==T_hi
    [lny,sigma,tau,phi] = gmpe(index,M,R,Zhyp,mechanism,arc,DeltaC1,Vs30);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M,R,Zhyp,mechanism,arc,DeltaC1,Vs30);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M,R,Zhyp,mechanism,arc,DeltaC1,Vs30);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
%% DEPTH FUNCTION
fdepth = t11*(min(Zhyp,120)-60)*Fevent;
...
%% fsite
PGA1000 = getPGA1000(rrup,rhypo,M,Zhyp,mechanism,DeltaC1,Fevent,FFABA);

if Vs30<Vlin
    fsite = t12*log(Vsa/Vlin)-b*log(PGA1000+c)+b*log(PGA1000+c*(Vsa/Vlin).^n);
else
    fsite = (t12+b*n)*log(Vsa/Vlin);
end
