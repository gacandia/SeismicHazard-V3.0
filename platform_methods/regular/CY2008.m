...
if isnan(Z10)
    % Regression gives the value of Z10 in meters
    Z10 = exp(28.5-3.82/8*log(VS30^8+378.8^8));
else
    Z10 = Z10*1000; % Z10 must be input in kilometers
end
...
if T_lo==T_hi
    [lny,sigma,phi] = gmpe(index,M,Rrup, Rjb, Rx, Ztor, dip, Z10, SOF, event, VS30, Vs30type);
    tau              = sqrt(sigma.^2-phi.^2);
else
    [lny_lo,sigma_lo,sig_lo] = gmpe(index,  M,Rrup, Rjb, Rx, Ztor, dip, Z10, SOF, event, VS30, Vs30type);
    [lny_hi,sigma_hi,sig_hi] = gmpe(index+1,M,Rrup, Rjb, Rx, Ztor, dip, Z10, SOF, event, VS30, Vs30type);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
