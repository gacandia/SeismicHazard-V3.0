function[lny,sigma,tau,phi]=Idini2016(To,M,Rrup,Rhyp,Zhyp,Vs30,Tn,mechanism,adjfun)
...
% site class (Table 1)
if Tn==0                   ,   classIDI=1;
elseif and(0<Tn,Tn<=0.2)   ,   classIDI=2;
elseif and(0.2<Tn,Tn<=0.4) ,   classIDI=3;
elseif and(0.4<Tn,Tn<=0.8) ,   classIDI=4;
elseif Tn>0.8              ,   classIDI=5;
elseif Tn==-1              ,   classIDI=6;
end

if T_lo==T_hi
    [log10y,sigma,tau,phi] = gmpe(index,M,R,Zhyp,classIDI,Vs30,mechanism);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M,R,Zhyp,classIDI,Vs30,mechanism);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M,R,Zhyp,classIDI,Vs30,mechanism);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
function[log10y,sigma,tau,phi]=gmpe(index,M,R,Zhyp,siteclass,Vs30,mechanism)
...
C = Co(index,:);
switch siteclass
    case 1, sTa  = 0; Vs30 = 1530; % Just to make sure FS=0;
    case 2, sTa  = C(1);
    case 3, sTa  = C(2);
    case 4, sTa  = C(3);
    case 5, sTa  = C(4);
