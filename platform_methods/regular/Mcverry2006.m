...
isadmisible = imag(To)==0 && or(To>=0 && To<=3,To==-1);
...
if isnumeric(media)
    if     media<180                   ,media='E';
    elseif and(180<=media,media< 270)  ,media='D';
    elseif and(270<=media,media< 360)  ,media='C';
    elseif and(360<=media,media<1500)  ,media='B';
    elseif 1500<=media                 ,media='A';
    end
end
...
if T_lo==T_hi
    [lny,sigma,tau,phi] = gmpe(index,M,Rrup,Zhyp,mechanism,media,rvol);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M,Rrup,Zhyp,mechanism,media,rvol);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M,Rrup,Zhyp,mechanism,media,rvol);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
switch mechanism
    case 'interface'       ,SI= 1; DS=0;   CS=1; if ischar(rvol), rvol = str2double(rvol); end
    case 'intraslab'       ,SI= 0; DS=1;   CS=1; rvol=0;
    case 'strike-slip'     ,CN= 0; CR=0;   CS=0; if ischar(rvol), rvol = str2double(rvol); end
    case 'normal'          ,CN=-1; CR=0;   CS=0; if ischar(rvol), rvol = str2double(rvol); end
    case 'normal-oblique'  ,CN=-1; CR=0;   CS=0; if ischar(rvol), rvol = str2double(rvol); end
    case 'reverse'         ,CN= 0; CR=1;   CS=0; if ischar(rvol), rvol = str2double(rvol); end
    case 'reverse-oblique' ,CN= 0; CR=0.5; CS=0; if ischar(rvol), rvol = str2double(rvol); end
    case 'unspecified'     ,CN= 0; CR=0;   CS=0; if ischar(rvol), rvol = str2double(rvol); end
end
