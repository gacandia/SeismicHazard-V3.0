function[lny,sigma,tau,phi]=Youngs1997(To,M,Rrup,Zhyp,VS30,mechanism,adjfun)
...
switch VS30
    case 'soil', isadmisible = imag(To)==0 && To>=0 && To<=4;
    case 'rock', isadmisible = imag(To)==0 && To>=0 && To<=3;
end
...
if T_lo==T_hi
    [lny,sigma] = gmpe(index,M,Rrup,Zhyp,VS30,mechanism);
else
    [lny_lo,sigma_lo] = gmpe(index,  M,Rrup,Zhyp,VS30,mechanism);
    [lny_hi,sigma_hi] = gmpe(index+1,M,Rrup,Zhyp,VS30,mechanism);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
