function[lny,sigma,tau,phi]=AtkinsonBoore2003(To,M,Rrup,Zhyp,VS30,mechanism,region,adjfun)

% Syntax : AtkinsonBoore2003 mechanism region                               

isadmisible = imag(To)==0 && To>=0 && To<=1/0.33;
if ~isadmisible
    lny   = [];
    sigma = [];
    tau   = [];
...
if isnumeric(VS30)
    if  VS30<180                      ,VS30='nehrpe';
    elseif and(180<=VS30,VS30<=360)   ,VS30='nehrpd';
    elseif and(360< VS30,VS30<=760)   ,VS30='nehrpc';
    elseif 760<VS30                   ,VS30='nehrpb';
    end
end
...
if T_lo==T_hi
    [log10y,sigma,tau,phi] = gmpe(index,M,Rrup,Zhyp,VS30,mechanism,region);
else
    [log10y_lo,sigma_lo,tau_lo] = gmpe(index,  M,Rrup,Zhyp,VS30,mechanism,region);
    [log10y_hi,sigma_hi,tau_hi] = gmpe(index+1,M,Rrup,Zhyp,VS30,mechanism,region);
    x          = log([T_lo;T_hi]);
