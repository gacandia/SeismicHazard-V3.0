...
isadmisible = imag(To)==0 && To>=0 && To<=5;
...
if T_lo==T_hi
    [lny,sigma,tau,phi] = gmpe(index,M,Rrup,Zhyp,Vs30,mechanism);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M,Rrup,Zhyp,Vs30,mechanism);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M,Rrup,Zhyp,Vs30,mechanism);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
switch lower(mechanism)
    ...
    case 'normal'
    case 'normal-oblique'
    case 'reverse',           SR = Coeff1(6);
    case 'reverse-oblique',   SR = Coeff1(6);
    case 'unspecified'
end
