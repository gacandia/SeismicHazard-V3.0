...
if T_lo==T_hi
    [lny,sigma] = gmpe(index,M, Rrup, media, SOF, location, sigmatype);
else
    [lny_lo,sigma_lo] = gmpe(index,  M, Rrup, media, SOF, location, sigmatype);
    [lny_hi,sigma_hi] = gmpe(index+1,M, Rrup, media, SOF, location, sigmatype);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
